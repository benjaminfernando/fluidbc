...
void FluidPoint::updateNewmark(double dt) {
 /*
  ...
  //  }
    if (mFluidSurf) {
        std::cout<<"Fluid Point detects fluid surface"<<std::endl;
    }

    //if (mFluidSurf & mIsFluid) {
    //  std::cout<<"FS and F"<<std::endl;
    // }

  */
    if(mFluidSurf) {
      resetZero();
    //  std::cout<<"Fluid point reset zero called"<<std::endl;
...
std::string FluidPoint::verbose() const {
    ...
    std::cout<<mMass->verbose()<<std::endl;
}
